...
export interface ProductiveWindows {
...
function predictTaskCompletion(
  ...
  historicalTasks: any[],
  timeEntries: any[]
): Omit<TaskPrediction, 'id' | 'name' | 'priority' | 'deadline' | 'estimate_minutes'> {
...
function identifyProductiveWindows(tasks: any[], timeEntries: any[]): ProductiveWindows {
...
// Generate comprehensive predictions and insights
export async function generatePredictions(timeframe: '7d' | '30d' | '90d'): Promise<{
  predictions: TaskPrediction[]
  ...
  try {
    ...
    const predictions: TaskPrediction[] = pendingTasks.map(task => {
    ...
    const schedulingSuggestions = generateSchedulingSuggestions(pendingTasks, predictions)
